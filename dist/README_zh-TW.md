...
- **需要的東西全部包在裡面**：安裝時不上網抓任何東西，所有內容（含任何第三方 binary）
  直接打進 MSI
- **簽章尚未到位**：未簽章的檔案在 WDAC 環境無法以發行者規則放行，
  且政府標案資安審查會退件
...
建置照樣成功但修正沒進 MSI，留指紋才回答得出「客戶手上那個 MSI 裡的腳本是哪一版」。

**本安裝檔目前未經 Authenticode 簽章。** 日後規劃透過開源專案的程式碼簽章
憑證方案申請；在那之前請以隨附的 `.sha256` 驗證完整性。手動信任的做法，
以及 WDAC / AppLocker 環境的處理方式，見
