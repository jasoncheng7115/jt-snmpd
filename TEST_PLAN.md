...
| 4.2.9 | 斷線網路磁碟存在時，poll 不得卡住（ 的 30 秒卡死） | walk 時間不變 | **[已修一半，2026-08-27]** 快照重建原本跑在 asyncio 事件迴圈上，實測 **229–245 ms**：每五秒有四分之一秒完全不回應，約 5% 的時間是聾的。而硬性規則 3 針對的正是「ctypes 呼叫無法中斷、斷線網路磁碟卡 30 秒以上」——在迴圈上那就是全面停擺。已改為 `run_in_executor`，重建期間照樣用前一份快照回應。**採集本身仍在同一條工作執行緒上串接**，單一 collector 卡住仍會拖住整批重建，那部分待做 |
| 4.2.10 | **代理服務必須在管理端逾時內回應**（net-snmp 預設 1 秒） | 任何時刻的回應延遲 < 1 秒 | **[已修]** 來源是 net-snmp 的 issue 194：agent 太慢時 manager 會忘記自己送過請求、重送，再因為訊息 ID 對不上而拒絕遲到的回覆，最後以「Timeout: No Response」失敗——**而 agent 其實是健康的**。這正是 4.2.9 那 245 毫秒窗口會造成的症狀 |
