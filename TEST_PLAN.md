...
**最終發行產物又重跑了一次**:程式碼在第一輪之後還加了布建檔、`user remove` 的儲存區修正
與 BOM 修正,所以那一輪測的不是要出貨的東西。以 `d7d58cf1…` 重跑 `.49`:**42/42**。
**驗產物不是驗來源,而且要驗「最後那一顆」。**

**最後再對 GitHub 上實際發行的那顆跑一次。** CI 建的產物與本機建的不是同一個檔案
(同一份原始碼、不同建置),而客戶拿到的是 CI 那顆。下載回來、在機器上核對
SHA-256 `314a43e5…` 與發行檔逐位元組相同,再跑同一套 42 項:**42/42**。
這一步在 1.1.1 那次救過場:當時本機一切正常,是下載回來實際升級才炸出 1603。
...
#### 清理必須依 ProductCode,不能依 MSI 路徑

`.wxs` 沒有寫死 `ProductCode`,所以**每建置一次 WiX 就換一個**。
`msiexec /x <剛建出來的 MSI>` 因此**對不上前一次建置裝上去的產品**:回 1605、什麼也沒做,
而接下來整輪測試就跑在一台從來沒被清乾淨的機器上。實際發生過,由 B 段擋下來。

**正式發行的 MSI 不會有這個問題**(一個版本只有一顆產物),這是拿本機建置反覆測試才有的。
腳本改為從解除安裝登錄機碼找出 `jt-snmpd*` 再依 ProductCode 移除;
**刻意不用 `Win32_Product`**,查詢那個類別會讓 Windows Installer 對每個已安裝產品做
reconfigure,慢而且已知會重啟服務。
