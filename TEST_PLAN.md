...
> **本機建置的 1.1.0 無法以 MSI 安裝(2026-08-27,調查中,與 v3 無關)。**
> `msiexec /qn` 以 1603 失敗,訊息為「服務啟動了但 30 秒內沒有回應迴路查詢」。
> 已排除的:程式碼(前景與服務模式都正常)、MSI 內容(抽出比對,exe 的 SHA-256
> 與能動的那顆完全相同)、安裝路徑(手動把同樣 286 個檔案複製進
> `C:\Program Files\jt-snmpd\`,服務完美啟動)、內建 SNMP 佔用 161 埠
> (安裝腳本會驗證它確實 Stopped/Disabled 才繼續)。
> 實際行為:服務走到 StartPending 之後直接 Stopped,事件 7024 回報服務特定錯誤
> `0x20000001`,而且**沒有**「服務已啟動」事件 —— 連 `SvcDoRun` 的第一行都沒執行到。
> **關鍵線索:同一批檔案,在 MSI 交易進行中啟動會失敗,交易結束後手動啟動就成功。**
> 上述 v3 項目是以手動部署驗證的,不是透過安裝程式。
