...
- **移除最後一個 SNMPv3 帳號,其實沒有移除。** `user remove` 印出「removed」,
  `user list` 卻還列著那個帳號,而且下次服務啟動又會把它註冊回去。
  金鑰儲存區會留一份前一版以備現行檔案讀不到,而讀取端只要現行檔案「沒有使用者」
  就退回去用那一份 —— **而「沒有使用者」正是移除最後一個帳號的正常結果**。
  會去移除唯一一個帳號的人,通常是因為認定那組憑證已經外洩,
  **而系統告訴他移除完成,那組憑證卻還能用。**

- **修復安裝會無聲換掉 community 與管理網段,監控就此停止。** 修復安裝,以及沒帶屬性派送的升級,
  都不會傳 `COMMUNITY`。安裝程式於是去內建 Windows SNMP Service 拿一個,
  **覆蓋掉 `config.json` 裡原本那個**。代理服務起來之後回應的是輪詢端不知道的 community,
  而**每一項檢查都還是通過**:服務在跑、安裝程式自己的回路探測用的是新值、
  `msiexec` 回傳 0。機器只是**安靜地不再被監控**。
  現在會保留機器上已經設定好的 community —— 順序是:操作人員明確指定的優先,
  其次是機器現有的,最後才是內建服務可以借給我們的。管理網段同樣處理:
  在一台網域控制站上,修復安裝把 `192.0.2.0/24` 換成了內建服務裡留著的兩個單一位址,
  **把 ACL 縮小了而且沒有講**。

- **不帶屬性的修復安裝會以 1603 中止。**
  `msiexec /i jt-snmpd.msi /qn REINSTALL=ALL REINSTALLMODE=vomus`,
  也就是一般執行修復安裝的方式,會卡在「必須指定管理網段」那個啟動條件;
  沒帶屬性派送的無人值守升級也一樣。**1603 完全沒有說明原因**,
  而原因根本不是「少了一個值」—— `config.json` 裡本來就有那些網段。
  現在這個條件在「產品已安裝」或「正在套用升級」時也會通過。
  **在一台全新機器上第一次無訊息安裝仍然必須指定**,訊息也會寫出要傳什麼:
  預設拒絕、絕不 Any/Any,這一點沒有改變。
  這與 1.1.1 修掉的 `v3_only` 重設是同一類:**安裝程式沒有收集的值,
  不可以從別的地方重新發明一個出來。**
...
- **圖形升級會要求操作人員關閉 Windows Event Log。** 它跳的是**兩個**
  ...
  Windows Installer 去停一個有其他服務相依的系統服務。

  原因在我們自己:pywin32 註冊事件來源時,把 `EventMessageFile` 指向它自己的
  `servicemanager.pyd`,而那個檔案**就在安裝目錄裡**;Event Log 服務為了格式化
  我們的訊息會載入它並持有 handle,於是升級要取代那個目錄時就發現檔案在使用中。
  現在事件來源改註冊到 `%SystemRoot%\System32\EventCreate.exe`,
  **Event Log 服務載入的東西沒有一樣是我們的**。安裝程式與代理服務兩邊都會寫這個註冊,
  因為從 1.1.2 升上來的機器帶著舊的機碼,必須在檔案被取代**之前**就改好。

  這個缺陷 1.0.0 就有,而且所有自動檢查都看不到它:`/qn` 不會問任何人,
  所以那一頁只存在於點兩下安裝的情境,而 40 項生命週期檢查全程走 `/qn`。
  是實機驅動精靈才發現的。

### 新增

- **SNMPv3 帳號可以由部署工具布建。** 為了跑 `user add` 而走訪一千台機器不是一個做法,
  而兩個看起來最直覺的替代方案都比想像中糟:**MSI 屬性會進 `msiexec` 記錄與
  事件 1033 / 11707**,並留在每一台被派送到的機器上;群組原則啟動指令碼則把密碼
  留在 SYSVOL,網域內每一台電腦都讀得到。改成在
  `%ProgramData%\jt-snmpd\provision.json` 放一個檔案:服務啟動時讀一次,
  轉成綁定該機器的 localized key 以 DPAPI 保存,然後**覆寫並刪除** ——
  **連讀不成功時也一樣刪**,所以打錯字不會讓明文永遠留在磁碟上。
  **怎麼把檔案送到機器上仍然是操作人員的責任**,
  [SNMPv3](https://jasoncheng7115.github.io/jt-snmpd/snmpv3_zh-TW.html)
  文件把這件事直說。**安裝程式仍然不接受任何 SNMPv3 參數,而且不會改。**

- **點兩下安裝的人現在找得到 SNMPv3 了。** 它從 1.1.0 就支援,在四台實機與正式
  LibreNMS 上驗過,卻沒有告訴任何人:精靈的設定頁只問管理網段與 community 就結束了,
  讀起來就像這是一個只有 v2c 的產品;完成頁只寫「Click the Finish button」;
  而 `user add` 這個字串在兩份 README 與專案首頁上**一次都沒出現過**。
  **一個沒人找得到的功能,跟沒有這個功能差不多** —— 這跟 1.1.1 修掉的是同一個毛病,
  只是低一層:`rate_pps`、`rate_burst`、`v3_only` 明明有效,卻不在那個負責列出設定的檔案裡。

  現在完成頁會寫出指令,設定頁會說明 SNMPv3 存在、以及為什麼不在那裡問,
  兩份 README 與專案首頁則補上布建步驟,以及要在很多台上做時怎麼用啟動指令碼處理。

  **安裝程式仍然不收任何 SNMPv3 參數,這一點不會改。** MSI 屬性會被寫進
  `msiexec` 的記錄檔,也會進 Windows 事件記錄的 **1033 與 11707** ——
  密碼一旦傳給安裝程式,就會以明文留在每一台被派送的機器上。
  community 是唯讀的存取權杖,風險等級不同;v3 金鑰不是。
  缺的是「在讀者看得到的地方把這件事講出來」,不是那個欄位。
