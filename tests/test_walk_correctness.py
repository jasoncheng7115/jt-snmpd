"""Walk correctness and response size.

Each of these is unacceptable, and every one of them happens in the MIB layer: a
GETNEXT loop, broken ordering, a duplicate OID, a wrong endOfMibView, a bulk
response too large to send. The snapshot + bisect architecture claims these are
structural guarantees rather than things to be maintained. This file checks the
...
def test_walk_returns_every_oid_exactly_once(agent, expected):
    ...
    assert len(got_oids) == len(set(got_oids)), "a duplicate OID appeared"
    assert got_oids == list(expected.oids), "the walk disagrees with the snapshot"
...
def test_walk_is_strictly_lexicographically_ordered(agent):
    ...
    assert not bad, f"ordering is broken or repeated; first 3: {bad[:3]}"


def test_walk_terminates_and_does_not_loop(agent, expected):
    """A GETNEXT loop shows up as a walk that never ends; the count bounds it."""
    got = _walk(agent)
    assert len(got) == len(expected), f"expected {len(expected)}, got {len(got)}"
...
def test_walk_result_is_identical_regardless_of_max_repetitions(agent, expected, maxrep):
    """max-repetitions changes how many packets there are, not what is in them.

    The server caps max-repetitions at 25 and ignores anything larger. After a
    truncation the manager simply asks again, so the result set has to be
...
def _raw_getbulk(port: int, oid_tuple: tuple[int, ...], max_reps: int) -> bytes:
    """Build a GETBULK by hand, send it over raw UDP, and measure the reply.

    This is the only way to check the cap for real: the net-snmp command-line
    tools do not report packet size, and a response past the MTU is fragmented
    and dropped by the firewall. The symptom is "LibreNMS intermittently gets no
    data".
    """
...
def test_response_never_exceeds_mtu_budget(agent, max_reps):
    ...
    assert len(data) <= MAX_RESPONSE_BYTES, (
        f"the response to max-repetitions={max_reps} was {len(data)} bytes, "
        f"over {MAX_RESPONSE_BYTES}, which causes IP fragmentation"
)


def test_oversized_max_repetitions_is_capped_not_rejected(agent):
    """Asking for 10,000 has to return fewer but valid results, not an error and
    ...
    assert 0 < len(vbs) <= 25, f"should be capped at 25 or fewer, got {len(vbs)}"


def test_end_of_mib_view_is_not_padded(agent, expected):
    """At the end of the MIB, the response must not be padded to
    max-repetitions with endOfMibView.

    pysnmp's own BulkCommandResponder does exactly that: a 200-entry tree was
    measured returning 225 rows. Every subtree's last packet wastes bandwidth,
    ...
    assert len(vbs) == 1, f"the closing response should hold one endOfMibView, got {len(vbs)}"


# --- GET / GETNEXT boundaries -----------------------------------------------
